...
<head>
    ...
    <title>Tambah Barang | KampusPinjam</title>
        ...
        <main class="flex-1 flex flex-col bg-white">
            ...
            <div class="flex-1 px-8 pb-8 bg-white flex justify-center items-start">
                <div class="bg-white rounded shadow border w-full max-w-3xl mt-12 p-8">
                    ...
                    @endif
                    <form method="POST" action="{{ route('tambah_barang.store') }}">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                            <div>
                                <label class="block font-semibold mb-2">Kode Barang</label>
                                <input type="text" name="kode_barang" value="{{ $nextKode }}" 
                                    class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" readonly>
                            </div>
                            <div>
                                <label class="block font-semibold mb-2">Nama Barang</label>
                                <input type="text" name="nama_barang" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                            <div>
                                <label class="block font-semibold mb-2">Merk</label>
                                <input type="text" name="merk" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                            </div>
                            <div>
                                <label class="block font-semibold mb-2">Jenis</label>
                                <input type="text" name="jenis" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                            </div>
                        </div>
                        <div class="mb-6">
                            <label class="block font-semibold mb-2">Unit</label>
                            <input type="number" name="unit" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                        </div>
